(function ($, undefined) {
    ...
    coreEasyui.ajaxError = function (callback) {
         ...
         * Author: chen.cheng
         * Description: 增加了错误提示时对组件的识别功能
         * Modify Start
         */
        /*************** Add Start *****************/
        $.fn.datagrid.defaults.onLoadError.pluginName = "datagrid";
        /*************** Add End *******************/
        /* Modify End */
    ...
    var onLoadError = function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        */
         ...
         * Date: 2016.3.10 17:10:22
         * Author: chen.cheng
         * Description: 针对返回的报错信息是个页面代码的情况进行处理，增加了iframe盒子
         * Modify Start
         */
        /*************** Add Start *****************/
        var reg = /(?=\<.*?DOCTYPE)/;
        var iframe = reg.test(XMLHttpRequest.responseText) && arguments.callee.pluginName == "datagrid";
        if(iframe) var opts = $(this).datagrid("options");
        /*************** Add End *******************/
        ...
        var msg = (XMLHttpRequest && !$.string.isNullOrWhiteSpace(XMLHttpRequest.responseText) ?
                /*************** Add Start *****************/
                iframe?
                "<iframe name='error-message' frameborder='0' width='100%' height='400' marginwidth='0px' marginheight='0px' scrolling='auto'></iframe>"
                /*************** Add End *******************/
                :"<div class=\"error-message-title\">如果该问题重复出现，请联系您的系统管理员并反馈该故障。<br />" +
                "错误号：" + XMLHttpRequest.status + "(" + XMLHttpRequest.statusText + ")；</div>" + 
                "<div class=\"error-message-content\"><hr />" + XMLHttpRequest.responseText + "</div>"
        ...
        var win = iframe? coreEasyui.messager.alert("错误提醒", msg, "") : coreEasyui.messager.alert("错误提醒", msg, "error"),
            opts = win.window("options"), panel = win.window("panel"), 
            width = panel.outerWidth(), height = panel.outerHeight();
        /*************** Add Start *****************/
        if(iframe){
            var iframe = top.frames["error-message"];
            iframe.document.open();
            iframe.document.write(XMLHttpRequest.responseText);
            iframe.document.close();
            win.window("resize", { width: 800 });
        }else{
        /*************** Add End *******************/
        /* Modify End */
            var _w=$(window).width()-20,_h=$(window).height()-200;
