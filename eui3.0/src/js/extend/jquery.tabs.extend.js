(function ($, undefined) {
    ...
    function updateTab(target, param) {
        ...
        /**
        * Add Property: 默认显示操作进度提醒，可强制不显示操作进度提醒
        * Author: wu.hao
        * Date: 2016/01/21
        */
        if(panelOpts.showMessager==false){
        /* Modify End */
        }else if ((!$.string.isNullOrWhiteSpace(panelOpts.href) || !$.string.isNullOrWhiteSpace(panelOpts.content)) && (panelOpts.selected || tabs.tabs("getSelected") == param.tab) && !panelOpts.iniframe) {
        ...
        /**
         * Description: Solve the tabs's type update conflict without title.
         * Author: chen.c1
         * Date: 2015/11/5
         * Add tabType param for refreshTab by wu.h1 on 2015/11/26
         * Modify Start
         */
        var tabType = param.type ? param.type : 'header';
        var ret = _updateTab.call(tabs, tabs, { tab: param.tab, options: panelOpts, type: tabType });
        /**
         * Modify End
         */
    ...
    function refreshTab(target, which) {
        ...
        /**
         * Add type param for updateTab method
         * Author: wu.h1
         * Date: 2015/11/26
         * Modify Start
         */
        // tabs.tabs("update", { tab: panel, options: panelOpts });
        tabs.tabs("update", { tab: panel, options: panelOpts, type: 'all' });
        /* Modify End */
