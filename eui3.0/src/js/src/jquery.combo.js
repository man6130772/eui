...
(function($){
	...
	function togglePanel(target){
		...
		var state = $.data(target, 'combo'),
			panel = state.panel,
			opts = state.options;
		if (!opts.searchBox&&panel.is(':visible')){
		/* Modify End */
			hidePanel(target);
		} else {
		...
		if(!opts.searchBox){$(target).combo('textbox').focus();}
		/* Modify End */
	}
    ...
    /**
	 * The focus event handler on input box
     * Fix focus event couldn't be bound and show panel bug
     * Author: wu.h1
     * Date: 2015/11/26
     */
	function inputFocusHandler(e){
		...
		var opts = state.options;
		var panel = state.panel;
        if (opts.autoShowPanel && panel.is(":hidden")) { $(target).combo("showPanel"); }
	}
    /* Modify End */
	...
	function showPanel(target){
		...
		if (palOpts.closed){
			...
			panel.panel('resize', {
				...
				/**
				 * Description: Fix the max-height lost bug
				 * Author: chen.c1
				 * Date: 2015/12/15
				 * Modify Start
				 */
			...
			if(opts.searchBox){
				setTimeout(function(){
					$(target).combo("panel").find(".panel-search .ipt")[0].focus();
				},200);
			}
			/* Modify End */
		}
		...
		(function(p){
			/**
			 * Description: 用resize代替延时的循环调用
			 * Author: wu.hao
			 * Date: 2016/01/21
			 * Modify Start
			 */
			p.movePanel=function(){
				if (p.is(':visible')){
					p.panel('move', {
						left:getLeft(),
						top:getTop()
			...
			p.movePanel();
			$(window).on('resize.combo',p.movePanel);
			/* Modify End */
		})(panel);
	...
	function hidePanel(target){
		...
		panel.panel('close');
		/**
		 * Description: 注销resize事件
		 * Author: wu.hao
		 * Date: 2016/01/21
		 * Modify Start
		 */
		$(window).off('resize.combo',panel.movePanel);
		/* Modify End */
	}
	...
	$.fn.combo = function(options, param){
		...
		return this.each(function(){
			...
			if (state){
				...
				/**
				 * Date: 2016/02/01 18:27:30
				 * Author: chen.c1
				 * Description: Fix the disabled combobox can't disabled problem.
				 * Modify Start
				 */
				// return;
				/* Modify End */
...
//				clear(this);
		...
		setValues: function(jq, values){
			return jq.each(function(){
		         ...
		         */
				var opts=$.data(this,"combo").options;
				$(this).next('.combo').find('.combo-value').attr('defaultValue',opts.originalValue);
		        /* Modify End */
			});
		},
