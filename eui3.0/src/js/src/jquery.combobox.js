...
//		param = param || {};
	...
	function create(target){
		...
		$(target).combo('panel').unbind().bind('mouseover', function(e){
			...
			/**
			 * Date: 2016/02/16 10:16:35
			 * Author: chen.c1
			 * Description: add the event mark to the target in order to identify events.
			 * Modify Start
			 */
			target.event = {
				type: e.type
	...
	$.fn.combobox = function(options, param){
		...
		return this.each(function(){
			...
			if (state.options.data){
		         ...
		         * Date: 2015/09/24
		         */
				state.options.data = $.extend(true, [], state.options.data);
		        /* Modify End */
	...
	$.fn.combobox.methods = {
		...
		setValue: function(jq, value){
			return jq.each(function(){
		         ...
		         * Date: 2015/09/24
		         */
				$(this).combo("options").originalValue = [value];
				$(this).next('.combo').find('.combo-value').attr('defaultValue', [value]);
		        /* Modify End */
			});
		},
		clear: function(jq){
	...
	$.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
		 ...
		 * Author: chen.c1
		 * Description: avoid use textbox's options 'prompt: 请选择...'.
		 * Modify Start
		 */
		prompt:'请选择...',
        /* Modify End */
		...
		filter: function(q, row){
			var opts = $(this).combobox('options');
			/**
			 * Date: 2016/02/15 10:17:56
			 * Author: chen.c1
			 * Description: change the search condition.
			 * Modify Start
			 */
