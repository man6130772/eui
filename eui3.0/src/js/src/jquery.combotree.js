	...
	function create(target){
		...
		tree.tree($.extend({}, opts, {
			...
			onLoadSuccess: function(node, data){
				...
				/**
				 * Description: Add the searchBox for combotree
				 * Author: chen.c1
				 * Date: 2015/12/14
				 * Modify Start
				 */
				if(opts.searchBox == true) {
					$(target).combotree('initSearchBox');
				}
				/* Modify End */
			},
	...
	function retrieveValues(target){
		...
		if (opts.multiple){
			...
			for(var i=0; i<nodes.length; i++){
				...
				var isLeaf = tree.tree('isLeaf', nodes[i].target);
				if(opts.showParentText){
					vv.push(nodes[i].id);
					ss.push(nodes[i].text);
				}else{
					if(isLeaf){
