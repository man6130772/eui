...
//				}, 0);
	...
	function fixRowHeight(target, index, forceFix){
        ...
        /**
         * Add Property: setTRHeight
         * set tr height when need, to load big data datagrid much more faster
         * Author: wu.han
         * Date: 2015/09/25
         */
		if(opts.setTRHeight){
...
//				total:0,
		...
		function createColumnHeader(container, columns, frozen){
	        ...
	        /**
	         * set datagrid header table height
	         * Author: he.ff
	         * Date: Unknown
	         */
		    var isOk = false;
...
//				}
	...
	function fixColumnSize(target, field){
        ...
        /**
         * fix columns's width aren't same after merged
         * Author: wu.han
         * Date: 2015/09/07
         */
	...
	function getColumnFields(target, frozen){
		/**
		 * Date： 2016/04/20 10:03:22
		 * Author： chen.c1
		 * Description: change the arithmetic of getColumnFields from 1.4.3 to 1.3.5 when the field isn't defined in the last line.
		 */
		/*************** Update Start **************/
		/*var opts = $.data(target, 'datagrid').options;
...
//		scrollTo(target, index);
	...
	function selectRow(target, index, notCheck){
	     ...
	     * Author: wuhao(wu.h3)
	     * Date: 2016/01/06
	     * Desc 解决firefox下跳行的bug 2016-01-06 wuhao
	     **************Start***************/
		// scrollTo(target, index);
        /***************End*****************/
	}
	...
	/**
	 * Show checked number in the pager area
	 * Author: wu.h1
	 * Date: 2016/01/29
	 */
	function showCheckedNumber(target){
	    var pager = $(target).parent().siblings('.datagrid-pager');
	    var checkedRows = $(target).datagrid('getChecked');
	    var checkedNumber = checkedRows.length;
	    var checkedText = '已选中'+checkedNumber+'条记录';
	    var checkedInfo = $('<div class="checked-numbers" style="float:right;height:30px;line-height:30px;margin-right:10px;"></div>');
	    if($('.checked-numbers', pager).length==0){
	        pager.find('.pagination-info').after(checkedInfo);
	    }
	    $('.checked-numbers', pager).text(checkedText);
	};
	/* Modify End */
	...
	function checkRow(target, index, notSelect){
		 ...
		 * Date: 2016/01/29
		 */
		if(opts.checkedNumber && opts.pagination){
		    showCheckedNumber(target);
		}
		/* Modify End */
...
//			endEdit(target, i, true);
	...
	function request(target, params){
		...
		/**
	    * Description: 重置options.data，用于loaded时判断是否加载完数据
	    * Author: wu.hao
	    * Date: 2016/1/21
	    * Modify Start
	    */
		...
		var result = opts.loader.call(target, param, function(data){
			...
			/**
		    * Description: 当没有结果集返回的时候，loaded会重复触发loadData
		    * Author: wu.hao
		    * Date: 2016/1/21
		    * Modify Start
		    */	
	...
	$.fn.datagrid = function(options, param){
		...
		return this.each(function(){
			...
			bindEvents(this);
			/**
			 * Date: 2016/03/21 17:59:26
			 * Author: chen.cheng
			 * Description: 设置宽高后再填充数据会丢失正常宽高
			 */
			/*************** Delete Start **************/
			// setSize(this);
			/*************** Delete End ****************/
			...
			*/
			var data;
			if (opts.data && opts.data.rows) {
			    data = opts.data;
			/**
			 * Date: 2016/03/21 17:55:42
			 * Author: chen.cheng
			 * Description: 准对有些data不规范，数据是存在的，但是没有写进rows里面，此处做了兼容处理
			 */
			/*************** Add Start *****************/
			}else if( opts.data && opts.data.length > 0 ){
				var _data = {};
				_data.total = opts.data.length;
				_data.rows = opts.data;
				data = opts.data = _data;
			/*************** Add End *******************/
			}else {
			    data = $.fn.datagrid.parseData(this);
			}
		...
		function getEditorConf(name){
			...
			return {
				init: function(container, options){
			         ...
			         */
	                options = options || {};
	                input.attr("disabled", options.disabled || false);
			        /* Modify End */
...
//						msg.css('marginLeft', -msg.outerWidth()/2);
		...
		loaded: function(jq){
			return jq.each(function(){
		        ...
		        /**
		         * show no data tip when there is no data
		         * Author: he.ff
		         * Date: 2015/01/22
		         */
		        var opts = $(this).datagrid("options");
		        if (!opts.data && $(this).datagrid("getRows").length <= 0) {
		        	var msgContent = $(this).siblings('.datagrid-empty-msg');
		            if (opts.emptyMsg != "" && msgContent.length == 0) {
					...
					*/
		            ...
		            $('.datagrid-empty-msg', panel).remove();
		        }
		        /* Modify End */
			});
		},
		...
		renderRow: function(target, fields, frozen, rowIndex, rowData){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (col){
			         ...
			         */
			        //@before
                    // var value = eval("rowData['" + field.replace(/\./g, "']['") + "']");
                    //@before end

                    //@after
                    var args=(field||'').split(/\./g);
					var value=rowData[args[0]];
					if(args[1]){
						value=value[args[1]];
					}
					//@after end
