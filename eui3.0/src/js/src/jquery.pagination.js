...
(function($){
	...
	function refreshData(target, param){
		...
		if (ps.length){
			/*
		     * Description: Solve the problem of missing the node agination-page-list's value when options.pageSize isn't exist in options.pageList.
		     * Author: chen.c1
			...
			var isExist = false;
			$.each(opts.pageList, function(i, n){
				if(opts.pageSize == n){
					isExist = true;
					return false;
				}
