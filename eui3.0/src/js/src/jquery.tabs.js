	...
	function setSize(container, param) {
		...
		if (opts.tabPosition == 'left' || opts.tabPosition == 'right'){
			...
			if (opts.showHeader){
				...
				/**
				 * Description: Clear the padding's and border's width when set "showHeader" to "false"
                 * Author: he.ff
                 * Date: 2014/10/30
                 * Modify Start
                 */
...
//					selectTab(container, getLiIndex(li));
	...
	function updateTab(container, param){
		...
		if (param.type == 'all' || param.type == 'body'){
	        /**
	         * Fix refreshTab method for tab.extend.js
	         * Author: wu.h1
	         * Date: 2015/11/26
	         * Modify Start
	         */
			// pp.panel();
			pp.panel($.extend(opts, param.options, {
				iconCls: (param.options.icon ? param.options.icon : undefined)
			}));
        	/* Modify End */
		}
...
//		}
	...
	$.fn.tabs.methods = {
		options: function(jq){
			...
			/**
			 * Description: Setting the default tab as 0 not -1
	         * Author: chen.c1
	         * Date: 2015/9/21
	         * Modify Start
	         */
	...
	$.fn.tabs.defaults = {
		...
		/**
		 * Description: Setting the default tabHeight as 34 not 27
         * Author: chen.c1
         * Date: 2015/9/21
         * Modify Start
         */
