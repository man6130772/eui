...
(function($){
	...
	function buildTextBox(target){
		...
		if (opts.multiline){
			...
			$('<input type="'+opts.type+'" class="textbox-text combo-text" autocomplete="off">').prependTo(tb);
		}
	...
	function setSize(target, width){
		...
		addon.css({
			 ...
			 * Date：2016/01/29
			 * Author: chen.c1
			 * Description: fix the textbox calculate error
			 * Modify Start
			 */
		...
		input.css({
			width: tb.width() - icons.length * addon.outerWidth() - btn._outerWidth() + 'px',
			height: tb.height()+'px'
		});
		/* Modify End */
	...
	function bindEvents(target){
		...
		if (!opts.disabled && !opts.readonly){
			...
			for(var event in opts.inputEvents){
		        /**
		         * Fix keydown event couldn't be bound bug
		         * Author: chen.c1
		         * Date: 2015/11/18
		         */
		        event = event == "keydown"? (navigator.userAgent.indexOf("Firefox")>0? "keyup" : "keydown") : event;
				input.on(event+'.textbox', {target:target}, opts.inputEvents[event=event=="keyup"?"keydown":event]);
		        /* Modify End */
			}
		}
	...
	function initText(text, value){
		var $text=$(text);
		var opts = $text.textbox('options');
		var input = $text.textbox('textbox');
		value = value == undefined ? '' : String(value);

		if ($text.textbox('getText') != value){
			// opts.value = value;
			input.val(value);
		}
		opts.value = value;
...
//	}
	...
	$.fn.textbox.methods = {
		...
		/**
	    * 增加一个initText方法，只设置text不执行验证
	    * Author: wu.hao
	    * Date: 2016/01/28
	    */
		initText:function(jq, value){
			return jq.each(function(){
				initText(this,value);
			});
		},
		...
		initValue: function(jq, value){
			return jq.each(function(){
				...
				state.options.value = '';
				/**
			    * 初始化value的时候使用initText方法
			    * Author: wu.hao
			    * Date: 2016/01/28
			    */
				$(this).textbox('initText', value);
				/* Modify End */
