...
(function($){
	...
	function bindEvents(target){
		...
		for(var event in opts.events){
			...
			/**
			 * Description: Shield the function in JQuery for the problem of only display element can trigger "focus" event.
			 * 				Otherwise save the function "$.event.special.focus.trigger" in "$.event.special.focus._trigge".
		     * Author: chen.c1
		     * Date: 2016/01/12
		     **************Start***************/
			if(event == "focus" && $.event.special.focus.trigger){
				$.event.special.focus._trigger = $.event.special.focus.trigger;
	...
	function focusEventHandler(e){
		...
		var box = $(target);
		/**
		 * Description: Cancel this line for some element with 'readonly' need validate again. 
	     * Author: chen.c1
	     * Date: 2016/01/12
	     **************Start***************/
		...
		(function(){
            ...
            * Date: 2016/03/10
            **/
			if (state.validating  /**Start***/ && target.parentElement /***End***/){
	...
	function showTip(target){
		...
		/**
		 * Description: add a new param "none" to the validatebox's property "tipPosition" used for hiding the checking box
         * Author: he.ff
         * Date: 2015/3/20
         * Modify Start
         */
	    if (opts.tipPosition == "none") {
	       return;
        }
        //Modify End
...
//			bindEvents(this);
			/**
			 * Description: Cancel loading method "validate" used for checking value in the process of initialization
	         * Author: chen.c1
	         * Date: 2015/9/22
	         * Modify Start
	         */
