...
(function($) {
    function moveWindow(target, param) {
        ...
        /**
         * Description: Limit the window's position(left and top) less than 0px
         * Author: he.ff
         * Modify Start
         */
        state.options.left = state.options.left < 0 ? 0 : state.options.left;
        state.options.top = state.options.top < 0 ? 0 : state.options.top;
        /* Modify End */
    ...
    function setProperties(target) {
        ...
        state.window.draggable({
            ...
            onStartDrag: function(e) {
                ...
                /**
                 * Description: Cancel the virtual frame that appears in dragging
                 * Author: chen.c1
                 * Date: 2015/9/21
                 * Modify Start
                 */
            ...
            onStopDrag: function(e) {
                ...
                state.proxy.remove();
                state.proxy = null;*/
                state.window.removeClass("window-panel-proxy");
                $(target).window("move");
                /* Modify End */
            }
        });
    ...
    $.fn.window.methods = {
        ...
        open: function(jq, param) {
            $.fn.panel.methods.open(jq, param);
            var state = jq.data('window');
            var options=state.options;
            if (options.autoHCenter) {
                jq.each(function() {
                    hcenter(this);
                });
            }
            if (options.autoVCenter) {
                jq.each(function() {
                    vcenter(this);
                });
            }
            if (options.autoHCenter || options.autoHCenter) {
                $.fn.window.methods.move(jq);
            }
        },
        /***************End****************/
        window: function(jq) {
